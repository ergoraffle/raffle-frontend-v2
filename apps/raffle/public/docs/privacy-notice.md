...
This Privacy Notice explains how data is handled when you use the Rafflora interface (the **"Interface"**) at rafflora.com and interact with the Rafflora smart contracts on the Ergo Blockchain (the **"Protocol"**). It should be read together with the Rafflora Terms of Service (v1.1.0).

Rafflora is free, open-source, non-custodial software. There are no user accounts, and the contributors who publish the code do not build identity profiles or sell your personal data. For the limited off-chain technical data processed by the official Interface, the party operating rafflora.com acts as a data controller (see Section 10).
